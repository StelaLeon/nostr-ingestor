...
import com.zoomin.earth.datalake.models.{EOSE, NostrDataEvent, NostrEvent, NostrFilterAuthored, NostrFilterUnauthored}
...
given authoredParser: Parser[NostrFilterAuthored] with
  type In  = String
...
given unauthoredParser: Parser[NostrFilterUnauthored] with
  type In  = String
  type Out = Option[NostrEvent]

  def parse[NostrFilterUnauthored](message: String)(using logger: Logger[IO]): Option[NostrEvent] =
    io.circe.parser.parse(message) match
      case Left(value) =>
        logger.error(s"we failed parsing with the error: ${value.getMessage}")
        None
